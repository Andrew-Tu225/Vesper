...
def _make_parse_response(candidates: list[ContentSignalCandidate]) -> MagicMock:
    """Build a mock response that mimics openai.beta.chat.completions.parse output."""
    parsed = BatchClassifyResponse(candidates=candidates)
    choice = MagicMock()
...
async def test_batch_classify_returns_worthy_candidates():
    """Happy path: all messages are worthy — all candidates returned with correct fields."""
    ...
    assert len(result) == 1
    assert result[0].signal_type == "customer_praise"
    assert result[0].source_ids == ["17000000.000000", "17000000.000001"]
    assert "Acme" in result[0].summary


@pytest.mark.asyncio
async def test_batch_classify_filters_noise():
    """Noise messages are excluded — only worthy candidates are returned."""
    messages = _make_messages(
    ...
    assert len(result) == 1
    assert result[0].signal_type == "product_win"
    assert result[0].source_ids == ["17000000.000001"]
...
@pytest.mark.asyncio
async def test_batch_classify_empty_input_returns_empty():
...
async def test_batch_classify_drops_unknown_signal_types():
    ...
    mock_response = _make_parse_response(bad_candidates)
